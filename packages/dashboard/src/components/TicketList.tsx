...
export function TicketList({ store, className = '' }: { store: ChatStore; className?: string }) {
  return (
    <aside
      className={`panel w-full shrink-0 flex-col overflow-hidden rounded-2xl lg:w-80 xl:w-[340px] ${className}`}
    >
      {/* Search + tabs */}
      <div className="border-b border-slate-100 p-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-full border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-3 text-sm text-slate-800 placeholder:text-slate-400 outline-none transition focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-500/10"
          />
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        {store.loadingList && store.tickets.length === 0 && (
          <div className="p-6 text-center text-xs text-slate-400">Загрузка…</div>
        )}
        {!store.loadingList && store.tickets.length === 0 && (
