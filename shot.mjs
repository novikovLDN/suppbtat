...
await p.screenshot({path:'/tmp/dk2-desktop.png'});console.log('desktop');await p.close();
p=await b.newPage();await p.setViewport({width:390,height:844,isMobile:true,deviceScaleFactor:2});await setup(p);
await p.screenshot({path:'/tmp/dk2-mobile.png'});console.log('mobile');await p.close();
await b.close();
